...
public class CustomBootstrapDropDown extends AwesomeTextView implements View.OnClickListener, RoundableView, OutlineableView, PopupWindow.OnDismissListener {

  private static final String TAG = "CustomBootstrapDropDown";
  ...
  public CustomBootstrapDropDown(Context context) {
    super(context);
    this.context = (Activity) context;
    initialise(null);
  }

  public CustomBootstrapDropDown(Context context, AttributeSet attrs) {
    super(context, attrs);
    this.context = (Activity) context;
    initialise(attrs);
  }

  public CustomBootstrapDropDown(Context context, AttributeSet attrs, int defStyle) {
  ...
  private String[] getDataArray() {
    Map<String, DownloadPathology> saveMap = ESApplication.Companion.getSaveMap();

    List<String> list = new ArrayList<>();

    Map<String, Boolean> activeStdMap = ESApplication.Companion.getActiveMap();

    for (PType type : PType.values()) {

      if (activeStdMap.containsKey(type.getPname())) {
        try {
          if (activeStdMap.get(type.getPname())) list.add(type.getPname());
        } catch (NullPointerException e) {
          Log.i("Dropdown", "Could not find element in activeStdMap");
        }
      } else {
        list.add(type.getPname());
      }
    }

    for (String pname : saveMap.keySet()) {

      if ((activeStdMap.containsKey(pname))) {
        try {
          if (activeStdMap.get(pname)) list.add(pname);
        } catch (NullPointerException e) {
          Log.i("Dropdown", "Could not find element in saveMap");
        }
      } else {
        list.add(pname);
      }
    ...
    Log.i("DropDown", "saveMap: " + saveMap.keySet().toString());
  ...
  private void initialise(AttributeSet attrs) {
    ...
    try {
      ...
      if (isInEditMode()) {
        ...
        // TODO: Customize the loaded Strings

        dropdownData = getDataArray();//getContext().getResources().getStringArray(dataOrdinal);
      }
    } finally {
      a.recycle();
    }
  ...
  public void onClick(View v) {
    ...
    dropdownWindow.setFocusable(false);
    ...
    switch (expandDirection) {
      case UP:
        ...
        int x = location[0];
        int y = location[1];
        Log.i("DropDown", "Measured Height: " + getMeasuredHeight() + " DropDownHeight: " + dropDownViewHeight + " axisYOffset: " + axisYOffset * 3 + " Position in Screen: " + y);
        PopupWindowCompat.showAsDropDown(dropdownWindow, v,
            axisXOffset,
            -y,// - getMeasuredHeight() - axisYOffset, // * 3,
            gravity);
        break;
      case DOWN:
        PopupWindowCompat.showAsDropDown(dropdownWindow, v,
    ...
    fullScreenImmersive(dropdownWindow.getContentView());
    dropdownWindow.setFocusable(true);
    dropdownWindow.update();
  ...
  /**
   * A listener which provides methods relating to {@link CustomBootstrapDropDown}
   */
  public interface OnDropDownItemClickListener {

    /**
     * Called when an item is clicked in a {@link CustomBootstrapDropDown}
     *
  ...
  private static class DividerView extends androidx.appcompat.widget.AppCompatTextView {
